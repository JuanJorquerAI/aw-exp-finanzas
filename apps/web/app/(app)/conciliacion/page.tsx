...
export default function ConciliacionPage() {
  ...
  const [fileType, setFileType] = useState<"detallado" | "historico">(
    "detallado",
  );
  ...
  const { data: bankTxs = [], isLoading: loadingTxs } = useTransactions({
    source: "BANK_CSV",
  });
    ...
    <div className="p-8 space-y-8 w-full">
      <div>
        ...
        <p className="mt-0.5 text-xs dark:text-slate-500 text-slate-400">
          Importa cartolas BCI · Movimientos Detallado o Histórica
        </p>
      </div>
      ...
      <form
        ...
        className="rounded-lg border dark:border-slate-800 border-slate-200 dark:bg-slate-900/40 bg-white p-6 space-y-4"
      >
        <h3 className="text-sm font-medium dark:text-slate-200 text-slate-800">
          Subir cartola
        </h3>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            ...
            <select
              value={fileType}
              onChange={(e) =>
                setFileType(e.target.value as "detallado" | "historico")
              }
              className="w-full rounded-md border dark:border-slate-700 border-slate-200 dark:bg-slate-800 bg-white px-3 py-1.5 text-sm dark:text-slate-200 text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500"
            >
              <option value="detallado">
                Movimientos Detallado (recomendado)
              </option>
              <option value="historico">
                Cartola Histórica (meses anteriores)
              </option>
            </select>
          </div>
        </div>
      ...
      {result && (
        <div className="rounded-lg border dark:border-slate-800 border-slate-200 dark:bg-slate-900/40 bg-white p-5">
          <h3 className="mb-3 text-sm font-medium dark:text-slate-200 text-slate-800">
          ...
          <div className="flex gap-6">
            ...
            <Stat
              icon={
                <SkipForward className="h-4 w-4 dark:text-slate-500 text-slate-400" />
              }
              label="Duplicadas"
              value={result.skipped}
              color="dark:text-slate-400 text-slate-500"
            />
            <Stat
      ...
      {pending.length > 0 && (
        <div>
          <h3 className="mb-3 text-sm font-medium dark:text-slate-200 text-slate-800 flex items-center gap-2">
            ...
            <span className="rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 px-2 py-0.5 text-xs font-semibold">
              {pending.length}
            </span>
          </h3>
      ...
      {!loadingTxs &&
        bankTxs.filter((t: Transaction) => t.status !== "PENDING").length >
          0 && (
          <div>
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-sm font-medium dark:text-slate-200 text-slate-800">
                Importadas recientes
              </h3>
      ...
      {!loadingTxs && bankTxs.length === 0 && !result && (
        <p className="text-sm dark:text-slate-600 text-slate-400">
          No hay transacciones bancarias importadas aún.
        </p>
      )}
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b dark:border-slate-800 border-slate-100 dark:bg-slate-900 bg-slate-50">
            ...
            <th className="px-4 py-2.5 text-center font-medium dark:text-slate-400 text-slate-500">
              Estado
            </th>
        ...
        <tbody className="divide-y dark:divide-slate-800 divide-slate-100">
          {transactions.map((tx) => (
            ...
            >
              ...
              <td className="px-4 py-2.5 dark:text-slate-300 text-slate-700 max-w-xs truncate">
                {tx.description}
              </td>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    CANCELLED: {
      label: "Cancelada",
      cls: "bg-slate-500/10 dark:text-slate-500 text-slate-400",
    },
  };
