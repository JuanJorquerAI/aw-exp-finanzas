...
export default function ContrapartesPage() {
  ...
  const filtered = counterparties.filter(
    (c) => !search || cpMatchesQuery(c, search),
  );
  ...
  async function handleCreate() {
    ...
    await create({
      ...
      ...(newForm.razonSocial.trim() && {
        razonSocial: newForm.razonSocial.trim(),
      }),
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs dark:text-slate-500 text-slate-400">
            {counterparties.length} registros
          </p>
        </div>
        <button
        ...
        <div className="mb-5 rounded-lg border dark:border-slate-700 border-slate-200 dark:bg-slate-900/40 bg-slate-50 p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                placeholder="Ej: Juan Pérez / Hosting SPA"
                className={INPUT_CLS}
              />
            </div>
            <div>
              ...
              <input
                value={newForm.razonSocial}
                onChange={(e) =>
                  setNewForm((p) => ({ ...p, razonSocial: e.target.value }))
                }
                placeholder="Nombre legal (empresas)"
                className={INPUT_CLS}
              />
            </div>
            <div>
              ...
              <input
                value={newForm.rut}
                onChange={(e) =>
                  setNewForm((p) => ({ ...p, rut: e.target.value }))
                }
            ...
            <div className="col-span-2 flex items-center gap-2">
              <input
                ...
                onChange={(e) =>
                  setNewForm((p) => ({
                    ...p,
                    isPersonaNatural: e.target.checked,
                  }))
                }
          ...
          <div className="mt-3 flex gap-2">
            <button
              ...
              className="rounded-md bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 px-3 py-1.5 text-xs font-semibold text-white transition-colors"
            >
              {creating ? "Creando…" : "Crear"}
            </button>
            <button
              onClick={() => setShowNew(false)}
              className="rounded-md border dark:border-slate-700 border-slate-200 px-3 py-1.5 text-xs dark:text-slate-400 text-slate-500 transition-colors dark:hover:bg-slate-800 hover:bg-slate-100"
            >
              Cancelar
            </button>
          </div>
            ...
            <tbody className="divide-y dark:divide-slate-800 divide-slate-100">
              {filtered.map((cp) =>
                editId === cp.id ? (
                  <tr key={cp.id} className="dark:bg-slate-900/60 bg-slate-50">
                    ...
                    <td className="px-4 py-2">
                      <input
                        value={editForm.rut}
                        onChange={(e) =>
                          setEditForm((p) => ({ ...p, rut: e.target.value }))
                        }
                        placeholder="RUT"
                        className={INPUT_CLS}
                      />
                    ...
                    <td className="px-4 py-2">
                      <div className="flex gap-1.5">
                        <button
                          ...
                          disabled={updating}
                          className="rounded p-1 text-emerald-500 dark:hover:bg-slate-800 hover:bg-slate-100 transition-colors"
                        >
                          <Check className="h-3.5 w-3.5" />
                        </button>
                        <button
                          onClick={() => setEditId(null)}
                          className="rounded p-1 dark:text-slate-500 text-slate-400 dark:hover:bg-slate-800 hover:bg-slate-100 transition-colors"
                        >
                  ...
                  <tr
                    key={cp.id}
                    className="dark:hover:bg-slate-800/30 hover:bg-slate-50 transition-colors"
                  >
                    <td className="px-4 py-2.5">
                      ...
                      {cp.isPersonaNatural && (
                        <span className="ml-2 rounded px-1.5 py-0.5 text-[10px] bg-violet-500/10 text-violet-600 dark:text-violet-400">
                          PN
                        </span>
                      )}
                    </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td
                    colSpan={5}
                    className="px-4 py-8 text-center dark:text-slate-600 text-slate-400"
                  >
                    Sin resultados
                  </td>
                </tr>
