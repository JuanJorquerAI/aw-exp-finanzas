...
function TaxRow({ label, value, sub, highlight, dim }: TaxRowProps) {
  return (
    <div
      className={`flex items-center justify-between py-2.5 border-b dark:border-slate-800 border-slate-100 last:border-0 ${dim ? "opacity-60" : ""}`}
    >
      <div>
        <p
          className={`text-sm ${highlight ? "font-semibold dark:text-slate-100 text-slate-900" : "dark:text-slate-300 text-slate-700"}`}
        >
          {label}
        </p>
        {sub && (
      ...
      <p
        className={`text-sm font-mono tabular-nums ${highlight ? "font-bold text-amber-500" : "dark:text-slate-200 text-slate-800"}`}
      >
        {fmt(value)}
      </p>
    </div>
...
function MonthlyPanel({
  ...
  if (isLoading)
    return (
      <div className="text-xs dark:text-slate-500 text-slate-400 py-4">
        Calculando...
      </div>
    );
  if (!data) return null;
  ...
  return (
    <div className="rounded-xl dark:bg-slate-900 bg-white border dark:border-slate-800 border-slate-200 p-5">
      ...
      <div className="mb-4">
        <p className="text-xs font-semibold dark:text-slate-500 text-slate-400 uppercase tracking-wider mb-2">
          Bases
        </p>
        <TaxRow
          label="Ingresos afectos"
          value={b.incomeAfecta}
          sub="Facturas afectas emitidas"
        />
        <TaxRow
          label="Ingresos exentos"
          value={b.incomeExenta}
          sub="Facturas exentas emitidas"
          dim
        />
        <TaxRow
          label="Gastos con factura afecta"
          value={b.expenseAfectaFactura}
          sub="IVA crédito fiscal"
        />
      </div>
      ...
      <div className="mb-4">
        ...
        <TaxRow
          label="Débito fiscal"
          value={b.ivaDebito}
          sub="19% ingresos afectos"
        />
        <TaxRow
          label="Crédito fiscal"
          value={b.ivaCredito}
          sub="19% gastos con factura afecta"
          dim
        />
        <TaxRow
      ...
      <div className="mb-5">
        ...
        <TaxRow
          ...
          value={b.retencionHonorarios}
          sub="13.75% boletas recibidas"
        />
      </div>
    ...
    <div className="rounded-xl dark:bg-slate-900 bg-white border dark:border-slate-800 border-slate-200 p-5">
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          ...
          <div
            key={label}
            className="rounded-lg dark:bg-slate-800 bg-slate-50 border dark:border-slate-700 border-slate-200 p-3"
          >
            <p className="text-xs dark:text-slate-500 text-slate-400 mb-1">
              {label}
            </p>
            <p className="text-sm font-bold tabular-nums dark:text-slate-200 text-slate-800">
              {fmt(value)}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-6 gap-1">
        {data.map((m) => (
          <div key={m.month} className="text-center">
            <p className="text-xs dark:text-slate-600 text-slate-400 mb-1">
            ...
            >
              ...
              >
                {m.breakdown.total > 0
                  ? `${Math.round(m.breakdown.total / 1000)}k`
                  : "—"}
              </span>
            </div>
          </div>
    ...
    <div className="p-6 max-w-3xl">
      <div className="mb-6">
        ...
        <p className="text-sm dark:text-slate-500 text-slate-500 mt-1">
          Estimación PPM, IVA y retenciones — {companyCode} ·{" "}
        ...
        <div className="space-y-4">
          <div className="rounded-lg dark:bg-slate-800/50 bg-blue-50 border dark:border-slate-700 border-blue-200 p-3">
            <p className="text-xs dark:text-slate-400 text-blue-700">
              <strong>Nota:</strong> Los valores son <em>estimaciones</em>{" "}
              basadas en transacciones registradas. PPM ~10% (Primera
              Categoría), IVA 19%, Retención honorarios 13.75%. Solo incluye
              transacciones no canceladas del período.
            </p>
          </div>
