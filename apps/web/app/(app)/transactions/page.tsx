...
export default function TransactionsPage() {
  ...
  const transactions = allTxs.filter(
    (tx: Transaction) => filter === "all" || tx.type === filter,
  );
  ...
  const totalIncome = allTxs
    ...
    .reduce((s: number, t: Transaction) => s + Number(t.amountCLP), 0);
  const totalExpense = allTxs
    .filter((t: Transaction) => t.type === "EXPENSE")
    .reduce((s: number, t: Transaction) => s + Number(t.amountCLP), 0);

  // Group by date
  const grouped = txsWithBalance.reduce(
    (
      acc: Record<string, TransactionWithBalance[]>,
  ...
  return (
    <div className="p-8 w-full">
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs dark:text-slate-500 text-slate-400">
            {companyCode} · {allTxs.length} movimientos
          </p>
        </div>
      ...
      <div className="mb-4 flex rounded-md border dark:border-slate-700 border-slate-200 overflow-hidden w-fit text-xs">
        {(
          [
            ["all", "Todos", allTxs.length],
            [
              "INCOME",
              "Ingresos",
              allTxs.filter((t: Transaction) => t.type === "INCOME").length,
            ],
            [
              "EXPENSE",
              "Egresos",
              allTxs.filter((t: Transaction) => t.type === "EXPENSE").length,
            ],
          ] as [TxFilter, string, number][]
        ).map(([v, label, count]) => (
          <button
            key={v}
            onClick={() => setFilter(v)}
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b dark:border-slate-800 border-slate-100 dark:bg-slate-900 bg-slate-50">
                ...
                <th className="px-4 py-2.5 text-center font-medium dark:text-slate-400 text-slate-500 w-12">
                  Docs
                </th>
                <th className="px-4 py-2.5 text-right font-medium dark:text-slate-400 text-slate-500 w-32">
                  Monto
                </th>
                <th className="px-4 py-2.5 text-right font-medium dark:text-slate-400 text-slate-500 w-32">
                  Saldo
                </th>
                <th className="px-4 py-2.5 text-center font-medium dark:text-slate-400 text-slate-500 w-24">
                  Estado
                </th>
              ...
              {dates.map((date) => (
                <React.Fragment key={date}>
                  <tr className="dark:bg-slate-900/60 bg-slate-50/80 border-b dark:border-slate-800 border-slate-100">
                    <td
                      colSpan={10}
                      className="px-4 py-1.5 text-[10px] font-semibold uppercase tracking-wider dark:text-slate-500 text-slate-400"
                    >
                      {fmtDate(date)}
                    </td>
                  </tr>
...
function TxRow({
  ...
  return (
    <tr className="border-b dark:border-slate-800/60 border-slate-100 dark:hover:bg-slate-800/30 hover:bg-slate-50 transition-colors">
      <td className="px-4 py-2.5 dark:text-slate-500 text-slate-400 tabular-nums">
        {fmtDate(tx.date)}
      </td>
      ...
      <td className="px-4 py-2.5 text-right tabular-nums text-xs font-mono">
        <span
          className={cn(
            "font-semibold",
            tx.balance >= 0 ? "text-emerald-500" : "text-rose-500",
          )}
        >
