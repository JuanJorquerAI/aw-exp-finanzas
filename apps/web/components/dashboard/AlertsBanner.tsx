...
export function AlertsBanner({
  ...
  const overdueIncome = allPending.filter(
    (t) =>
      t.type === "INCOME" &&
      getAgingStatus(effectiveDueDate(t), today) === "OVERDUE",
  );
  const overdueExpense = allPending.filter(
    (t) =>
      t.type === "EXPENSE" &&
      getAgingStatus(effectiveDueDate(t), today) === "OVERDUE",
  );
  const overdueIncomeTotal = overdueIncome.reduce(
    (s, t) => s + Number(t.amountCLP),
    0,
  );
  const overdueExpenseTotal = overdueExpense.reduce(
    (s, t) => s + Number(t.amountCLP),
    0,
  );
  ...
  return (
    <div className="space-y-2">
      {overdueExpense.length > 0 && (
        ...
        >
          <span className="font-semibold">
            {overdueExpense.length} CxP vencida
      ...
      {overdueIncome.length > 0 && (
        ...
        >
          <span className="font-semibold">
            {overdueIncome.length} CxC vencida
            {overdueIncome.length !== 1 ? "s" : ""}
          </span>
          <span className="dark:text-amber-300/70 text-amber-700/70 mx-1">
            ·
          </span>
      ...
      {bankPending.length > 0 && (
        ...
        >
          <span className="font-semibold">
            {bankPending.length} movimiento{bankPending.length !== 1 ? "s" : ""}{" "}
            bancario{bankPending.length !== 1 ? "s" : ""}
          </span>
...
function Alert({
  ...
  const cls = {
    ...
    amber:
      "dark:bg-amber-950/30 bg-amber-50 dark:border-amber-900 border-amber-200 dark:text-amber-400 text-amber-700",
  ...
  return (
    <Link
      href={href}
      className={cn(
        "flex items-center gap-2 rounded-lg border px-4 py-2.5 text-xs transition-opacity hover:opacity-80",
        cls,
      )}
    >
      {icon}
