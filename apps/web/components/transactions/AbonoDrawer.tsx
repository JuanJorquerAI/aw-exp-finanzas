...
function buildTimeline(
  ...
  const items: TimelineItem[] = [
    ...payments.map(
      (p): TimelineItem => ({ kind: "payment", date: p.paidAt, data: p }),
    ),
    ...auditLogs.map(
      (a): TimelineItem => ({ kind: "audit", date: a.createdAt, data: a }),
    ),
  ];
  return items.sort(
...
function AbonoForm({ tx, onClose }: AbonoFormProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (n > remaining + 0.01) {
      setError(
        `Máximo aboneable: ${fmtAmount(remaining.toString(), tx.currency)}`,
      );
      return;
    }
    setError("");
    ...
    <div className="flex flex-col gap-6">
      <div className="rounded-lg bg-slate-900 p-4 text-sm">
        <div className="flex justify-between">
          ...
          <span className="font-semibold tabular-nums dark:text-slate-100">
        ...
        <div className="flex justify-between mt-1">
          ...
          <span className="tabular-nums text-emerald-400">
            {fmtAmount(totalPaid.toString(), tx.currency)}
          </span>
        </div>
        <div className="mt-2 border-t border-slate-700 pt-2 flex justify-between">
          ...
          <span className="font-bold tabular-nums text-amber-400">
            {fmtAmount(remaining.toString(), tx.currency)}
          </span>
        </div>
      </div>
          ...
          <ul className="space-y-2">
            {timeline.map((item) => {
              ...
              return (
                <li
                  key={a.id}
                  className="rounded-md border border-indigo-900/50 bg-indigo-950/30 px-3 py-2"
                >
      ...
      {remaining > 0 && (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            <Label htmlFor="abono-amount" className="text-xs text-slate-400">
              Monto ({tx.currency})
            </Label>
            <Input
              id="abono-amount"
          ...
          <div className="space-y-1">
            <Label htmlFor="abono-note" className="text-xs text-slate-400">
              Nota (opcional)
            </Label>
            <Textarea
              id="abono-note"
      ...
      <SheetContent className="w-full sm:max-w-md overflow-y-auto">
        <SheetHeader className="mb-4">
          ...
          <p className="text-xs text-slate-500">
            {transaction.counterparty?.name ?? "Sin contrapartida"}
          </p>
        </SheetHeader>
