...
const INPUT_CLS =
  "w-full rounded-md border dark:border-slate-700 border-slate-200 dark:bg-slate-900 bg-white dark:text-slate-200 text-slate-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 dark:focus:ring-slate-600 focus:ring-slate-300 placeholder:dark:text-slate-600 placeholder:text-slate-400";
...
export function ReviewDrawer({
  ...
  const { mutateAsync: createCounterparty, isPending: creatingCp } =
    useCreateCounterparty();
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        className="w-full sm:max-w-md dark:bg-slate-950 bg-white overflow-y-auto"
        aria-describedby={undefined}
      >
        <SheetHeader className="mb-6">
          <SheetTitle className="dark:text-slate-100 text-slate-900">
            Revisar transacción
          </SheetTitle>
        </SheetHeader>
            ...
            {showNewCp ? (
              <div className="rounded-md border dark:border-slate-700 border-slate-200 p-3 space-y-2.5">
                <input
                  value={newCp.name}
                  onChange={(e) =>
                    setNewCp((p) => ({ ...p, name: e.target.value }))
                  }
                ...
                <input
                  value={newCp.rut}
                  onChange={(e) =>
                    setNewCp((p) => ({ ...p, rut: e.target.value }))
                  }
                ...
                <select
                  value={newCp.type}
                  onChange={(e) =>
                    setNewCp((p) => ({ ...p, type: e.target.value }))
                  }
            ...
            {(tx.documents?.length ?? 0) > 0 ? (
              <div className="space-y-1">
                {tx.documents!.map((link) => (
                  <div
                    key={link.id}
                    className="flex items-center justify-between rounded-md border dark:border-slate-700 border-slate-200 px-3 py-2 text-xs dark:bg-slate-900 bg-slate-50"
                  >
                    <span className="dark:text-slate-300 text-slate-700">
                      {link.document.type}{" "}
                      {link.document.number ? `#${link.document.number}` : ""}
                      {" · "}
                      <span className="dark:text-slate-500 text-slate-400">
                        {new Intl.NumberFormat("es-CL", {
                          ...
                          currency: "CLP",
                          maximumFractionDigits: 0,
                        }).format(Number(link.document.totalAmount))}
                      </span>
                    </span>
                    <button
            ...
            {allDocs.length > 0 && (
              <div className="flex gap-2">
                <select
                  ...
                  className="flex-1 text-xs rounded-md border dark:border-slate-700 border-slate-300 dark:bg-slate-900 bg-white px-2 py-1.5"
                >
                  ...
                  {(
                    ...
                    .filter(
                      (d) => !tx.documents?.some((l) => l.documentId === d.id),
                    )
                    .map((d) => (
            ...
            {(tx.notes?.length ?? 0) > 0 && (
              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                {tx.notes!.map((note) => (
                  <div
                    key={note.id}
                    className="rounded-md dark:bg-slate-900 bg-slate-50 border dark:border-slate-800 border-slate-200 px-3 py-2"
                  >
