...
const API_URL = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:41891";
...
export function cancelTransaction(id: string) {
  return fetchApi<Transaction>(`/transactions/${id}/cancel`, {
    method: "PATCH",
  });
}
...
export function addPayment(transactionId: string, dto: CreatePaymentInput) {
  return fetchApi<TransactionPayment>(
...
export function getTaxesMonthly(
  companyId: string,
  year: number,
  month: number,
) {
  return fetchApi<MonthlyTax>(
    `/taxes/monthly?companyId=${companyId}&year=${year}&month=${month}`,
  );
}

export function getTaxesAnnual(companyId: string, year: number) {
  return fetchApi<MonthlyTax[]>(
    `/taxes/annual?companyId=${companyId}&year=${year}`,
  );
}
...
export function createCounterparty(dto: {
  name: string;
  type: string;
  rut?: string;
  razonSocial?: string;
...
export function updateCategorizationRule(
  id: string,
  dto: Partial<CreateCategorizationRuleInput & { isActive: boolean }>,
) {
...
export function deleteCategorizationRule(id: string) {
  return fetchApi<{ deleted: boolean }>(`/categories/rules/${id}`, {
    method: "DELETE",
  });
}
...
export async function unlinkDocument(linkId: string) {
  const res = await fetch(`${API_URL}/transaction-documents/${linkId}`, {
    method: "DELETE",
  });
